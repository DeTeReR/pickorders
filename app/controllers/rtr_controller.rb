class RtrController < ApplicationController
  ...
  def update
    original = Card.where("color = ? and name = ?", params[:color], params[:name]);
	if params[:commit] == '<<'
	  params[:position] = 1
	end
	if params[:commit] == '>>'
	  params[:position] = Card.where("color = ?", params[:color]).order("position ASC")[-1].position;
    end
	if params[:commit] == '<'
	...
	target = Card.where("color = ? and position = ?", params[:color], params[:position]);
	  ...
	  if original.position != target.position
		if original.position < target.position
		  ...
		  for i in original.position+1..target.position
		    Card.where("color = ? and position = ?", params[:color], i)[0].update_attributes(:position => i-1)
		  end
		  ...
		  while i > target.position
		    Card.where("color = ? and position = ?", params[:color], i-1)[0].update_attributes(:position => i)
		    i -= 1;
		  end
		  original.position = target.position
		  original.save
